...
-- Also supports a staged rollout before the optional WhatsApp worker migration.
...
begin
  ...
  insert into public.bookings (quote_id,booking_reference,resource_id,pricing_resource_id,service_type,booking_date,start_at,end_at,buffer_mins,status,payment_status,amount,customer_name,customer_email,customer_phone,player_count,notes,expires_at,whatsapp_consent)
  values (q.id,'CP-'||upper(replace(gen_random_uuid()::text,'-','')),lane,equipment,(q.input->>'serviceType')::public.service_type,
    ...
    (p_customer->>'playerCount')::integer,p_customer->>'notes',now()+interval '31 minutes',p_customer->'whatsapp_consent') returning * into created;
