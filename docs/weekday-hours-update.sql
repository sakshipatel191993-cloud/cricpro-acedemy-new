-- Final approved schedule: Mon-Fri 09-22 (off-peak 12-16 only);
-- Sat-Sun 09-23 (all peak). Preserve prices, overrides and existing bookings.
begin;
...
select 'weekday-morning-peak-' || id, resource_id, 'Weekday Morning Peak (9am-12pm)',
...
insert into public.pricing_rules
  (id, resource_id, name, start_time, end_time, days, price, priority, active)
select 'weekend-all-peak-' || id, resource_id, 'Weekend Peak (9am-11pm)',
  '09:00'::time, '23:00'::time, array[0,6], price, priority, true
from public.pricing_rules
where active and start_time = '16:00' and end_time = '22:00'
  and days @> array[1,2,3,4,5]
on conflict (id) do nothing;

-- Remove weekend applicability from the superseded pricing bands.
...
where active and (0 = any(days) or 6 = any(days))
  and id not like 'weekend-all-peak-%';
commit;
