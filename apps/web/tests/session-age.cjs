...
assert.equal(sessionAgeOptions('13+').at(-1),40);
assert.equal(sessionAgeOptions('13+').length,28);
assert.equal(isSessionAgeAllowed(40,'13+'), true);
assert.equal(isSessionAgeAllowed(41,'13+'), false);
assert.equal(isSessionAgeAllowed(12,'13+'), false);
for (const group of [null,undefined,'','all','15-11','0-10','1-999','11-15 or 40','41+','8-41 years']) assert.equal(sessionAgeRange(group),null);
